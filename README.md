...
У розробці фронтенд і бекенд запускаються окремо.

У production-style режимі фронтенд спочатку збирається, після чого `backend` віддає готовий білд із `dropzone/dist`.
